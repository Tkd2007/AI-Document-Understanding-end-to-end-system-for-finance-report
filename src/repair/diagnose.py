...
#
# NÂNG TỪ 30 LÊN 300 NGÀY 05/09/2026, và đây KHÔNG phải nới lỏng cho êm: nó
# là hệ quả bắt buộc của việc nâng `MAX_CHANGES_MAC_DINH`. Đo được cùng ngày,
# ca VÔ NGHIỆM ở trần 3 tốn 13 giây với TT200 và 26 giây với TT99. Giữ giới
# hạn 30 giây thì phần lớn ca TT99 chạm giờ và trả `het_gio` thay vì
# `vo_nghiem`.
#
# 300 giây là biên hơn mười lần so với 26 giây đo được — đủ rộng cho tài liệu
# có nhiều ứng viên hơn bộ đo, mà vẫn chặn được những ca mài hàng giờ. Con số
# này từng là 2400 khi trần còn là 4; hạ theo khi trần hạ xuống 3.
#
...
# làm bảng cân đối được", còn `het_gio` không chứng minh gì cả. Một trần cao
# kèm giới hạn thời gian thấp vì vậy xoá sạch bằng chứng chống bịa trong khi
# mọi con số khác trông vẫn bình thường — hai hằng số này phải đi cùng nhau,
# đừng đổi một cái mà không xem cái kia.
TIME_LIMIT_S = 300.0
...
#
# ĐỔI 2 → 4 → 3 TRONG NGÀY 05/09/2026, và lịch sử ấy giữ nguyên ở đây vì nó
# giải thích con số hiện tại. Người chủ trì nâng lên 4, ngược với khuyến nghị
# giữ 2; rồi hạ xuống 3 sau khi bảng chi phí bên dưới được đo. Lý do nâng:
# trần 2 chặn thật ở đúng hai tài liệu của lượt 05/09 (`HPG_2022Q2` sai 3 ô,
# `HVG_2020Q1` sai 5 ô). Lý do khuyến nghị giữ: khoản lợi nhỏ, và trần đã
# được đăng ký là hạn chế của phương pháp.
#
...
#   trần 3 → 4,1 triệu,       13 giây     (TT99: 4,5 triệu, 26 giây)
...
# điểm chống bịa — nên phải trả.
#
# HẠ 4 → 3 NGÀY 05/09/2026 (muộn hơn), sau khi bảng trên được đo. Trần 4 cộng
# 5-10 giờ vào một lượt chấm 70 tài liệu vốn đã 9,5 giờ, còn trần 3 chỉ cộng
# khoảng 7 phút — rẻ hơn 70 lần. Trần 3 vẫn mở được `HPG_2022Q2` (sai 3 ô),
# tức vẫn lấy được phần lớn cái mà việc nâng trần nhắm tới; chỉ `HVG_2020Q1`
# (sai 5 ô) là ngoài tầm, mà nó vốn ngoài tầm cả ở trần 4.
MAX_CHANGES_MAC_DINH = 3
